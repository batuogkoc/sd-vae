...
class DynamicsVAE(BaseDynamicsModule):
    def __init__(self, *args, **kwargs):
        ...
        if self.hparams.kld_scaling_type == "geco":
            ...
            raise Exception(
                f"Wrong KLD scaling method: {self.hparams.kld_scaling_type}"
            )
        ...
        self.model = VAE(
            input_dim=self.hparams.model_input_size,
        ...
        if (
            self.hparams.use_supervision
            and self.hparams.sup_loss_type == "sigmoid_parametrized"
        ):
    ...
    def rollout(self, batch, start, rollout_size):
        ...
        while (
            output.size(1) < rollout_size
        ):  # keep rolling till we reach the required size
            ...
            if model_output_size < model_input_size:
                ...
                input_trajectory = torch.cat(
                    (
                        input_trajectory[:, -keep_from_input:, :],
                        output[:, -model_output_size:, :],
                    ),
                    dim=1,
                )
            else:
        ...
        return (
            output[:, :rollout_size, :],
            trajectory[:, output_start_point : (output_start_point + rollout_size), :],
        )
    ...
    def forward(self, batch):
        ...
        target_trajectory = trajectory[
            :,
            output_start_point : (output_start_point + self.hparams.model_output_size),
            :,
        ]
    ...
    def get_normalized_labels(self, labels):
        ...
        for k, v in labels.items():
            labels_norm[k] = (v - self.labels_min[k]) / (
                self.labels_max[k] - self.labels_min[k] + 1e-6
            )
        return labels_norm
    ...
    def update_geco_multiplier(self, C):
        ...
        self.geco_multiplier = torch.clamp(
            self.geco_multiplier, 1.0 / clipping, clipping
        )
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        for i in range(self.hparams.samples_per_batch_train):
            output_trajectory, target_trajectory, mu, logvar, factors = self.forward(
                train_batch
            )
            rec_loss = rec_loss + self.reconstruction_loss(
                output_trajectory, target_trajectory
            )
        ...
        if self.hparams.kld_scaling_type == "geco":
            ...
            self.log(
                "train/geco/C", C_curr, prog_bar=False, on_step=False, on_epoch=True
            )
            self.log(
                "train/geco/C_ma",
                self.C_ma.detach(),
            ...
            self.log(
                "train/geco/geco_multiplier",
                self.geco_multiplier,
                prog_bar=True,
                on_step=False,
                on_epoch=True,
            )
...
if __name__ == "__main__":
    ...
    if hparams.use_wandb:
        ...
        experiment_name = (
            f"in_{hparams.model_input_size}_out_{hparams.model_output_size}"
        )
    ...
    best_validation_callback = BestValidationCallback(
        hparams.monitor, hparams.use_wandb
    )
    test_end_callback = TestEndCallback(hparams.use_wandb)

    trainer = pl.trainer.Trainer.from_argparse_args(
        hparams,
        logger=logger,
        log_every_n_steps=1,
