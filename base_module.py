...
class BaseDynamicsModule(pl.LightningModule):
    ...
    def prepare_data(self, dataset_dir=os.environ["DATA_DIR_PHYSICS"]):
        ...
        if self.hparams.dataset == "pendulum_var_length":
            ...
            self.datasets["train"] = PendulumDataset(
                train_path,
                self.hparams.coordinates,
                self.hparams.noise_std,
                indexes=list(range(0, 9000)),
            )
            self.datasets["val"] = PendulumDataset(
                train_path,
                self.hparams.coordinates,
                self.hparams.noise_std,
                indexes=list(range(9000, 10000)),
            )
            self.datasets["test"] = [
                self.datasets["val"],
                PendulumDataset(
                    test_path1, self.hparams.coordinates, self.hparams.noise_std
                ),
                PendulumDataset(
                    test_path2, self.hparams.coordinates, self.hparams.noise_std
                ),
            ]
        if self.hparams.dataset == "pendulum-2":
            ...
            train_set = PendulumDataset(
                ...
                self.hparams.coordinates,
                self.hparams.noise_std,
                indexes=self.train_ind,
            )
            val_set = PendulumDataset(
                train_path,
                self.hparams.coordinates,
                self.hparams.noise_std,
                indexes=self.val_ind,
            )
            in_dist_test_set = PendulumDataset(
                train_path,
                self.hparams.coordinates,
                self.hparams.noise_std,
                indexes=self.test_ind,
            )
            ...
            test_path1 = os.path.join(dataset_dir, test_name1)
            test_path2 = os.path.join(dataset_dir, test_name2)
            self.datasets["train"] = LotkaVolterraDataset(
                train_path, self.hparams.noise_std
            )
            self.datasets["val"] = LotkaVolterraDataset(
                val_path, self.hparams.noise_std
            )
            self.datasets["test"] = [
                self.datasets["val"],
            ...
            if self.train_ind is None:
                ...
                self.val_ind = np.where((rand_ind > 0.80) & (rand_ind <= 0.90))[0]
                self.test_ind = np.where(rand_ind >= 0.90)[0]
            train_set = LotkaVolterraDataset(
                train_path, self.hparams.noise_std, indexes=self.train_ind
            )
            val_set = LotkaVolterraDataset(
                train_path, self.hparams.noise_std, indexes=self.val_ind
            )
            in_dist_test_set = LotkaVolterraDataset(
                train_path, self.hparams.noise_std, indexes=self.test_ind
            )
            ...
            if self.train_ind is None:
                ...
                self.val_ind = np.where((rand_ind > 0.80) & (rand_ind <= 0.90))[0]
                self.test_ind = np.where(rand_ind >= 0.90)[0]
            train_set = NBodyDataset(
                train_path, self.hparams.noise_std, indexes=self.train_ind
            )
            val_set = NBodyDataset(
                train_path, self.hparams.noise_std, indexes=self.val_ind
            )
            in_dist_test_set = NBodyDataset(
                train_path, self.hparams.noise_std, indexes=self.test_ind
            )
            ...
            val_path = os.path.join(dataset_dir, val_name)
            test_path = os.path.join(dataset_dir, test_name)
            self.datasets["train"] = PixelPendulumDataset(
                train_path, self.hparams.noise_std
            )
            self.datasets["val"] = PixelPendulumDataset(
                val_path, self.hparams.noise_std
            )
            self.datasets["test"] = [
                self.datasets["val"],
    ...
    def log_rec_losses(
        self, batch, stage, rec_loss_sizes, on_epoch=True, on_step=False
    ):
        ...
        for step in rec_loss_sizes:
            ...
            rec_loss = self.reconstruction_loss(
                output[:, (step - 1) : step], target[:, (step - 1) : step]
            )
            self.log(
                f"{stage}/rec/{step:04d}", rec_loss, on_step=on_step, on_epoch=on_epoch
            )
    ...
    def _compute_label_loss(self, labels, mu):
        ...
        if self.hparams.sup_loss_type == "sigmoid":
            pred_scaled = (
                torch.sigmoid(mu[:, : self.num_factors]) * (labels_max - labels_min)
                + labels_min
            )
            ...
            pred_scaled = (
                self.w1
                * torch.sigmoid(self.w2 * mu[:, : self.num_factors])
                * (labels_max - labels_min)
                + labels_min
            )
            label_loss = F.l1_loss(pred_scaled, labels)
            self.log(
                "train/sup/w2", self.w2, prog_bar=False, on_step=False, on_epoch=True
            )
            self.log(
                "train/sup/w1", self.w1, prog_bar=False, on_step=False, on_epoch=True
            )
            ...
            pred_scaled = (
                mu[:, : self.num_factors] * (labels_max - labels_min) + labels_min
            )
            label_loss = F.l1_loss(pred_scaled, labels)
